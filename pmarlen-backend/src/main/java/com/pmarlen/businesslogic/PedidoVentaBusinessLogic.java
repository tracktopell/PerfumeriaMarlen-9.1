...
public class PedidoVentaBusinessLogic {
	...
	public PedidoVenta crearPedidoCapturado(PedidoVenta pedidoVentaX, Usuario usuarioModifico) {
		...
		try {
			...
			logger.debug("-> antes pedidoVenta=" + pedidoVenta + ", detail size:" + detalleVentaPedidoOrigList.size() + ", {" + pedidoVenta.getCliente() + ", " + 
					pedidoVenta.getComentarios() + ", " + pedidoVenta.getDescuentoAplicado() + ", " + pedidoVenta.getFactoriva() + ", " + pedidoVenta.getFormaDePago() + ", " + pedidoVenta.getUsuario() + "}");
			em.persist(pedidoVenta);
			//em.flush();
			//em.merge(pedidoVenta);			

			logger.debug("-->> despues pedidoVenta=" + pedidoVenta + ": {" + pedidoVenta.getCliente() + ", " + pedidoVenta.getComentarios() + ", " + 
					pedidoVenta.getDescuentoAplicado() + ", " + pedidoVenta.getFactoriva() + ", " + pedidoVenta.getFormaDePago() + ", " + pedidoVenta.getUsuario() + "}");
			...
			if (detalleVentaPedidoOrigList != null) {
				for (PedidoVentaDetalle dvp : detalleVentaPedidoOrigList) {
					...
					//em.flush();
					logger.debug("\t->crearPedidoCapturado: detalleVentaPedido[" + dvp.getCantidad() + " x " + 
							dvp.getProducto() + "], pedidoVenta=" + pedidoVenta + ": persisted->>detalleVentaPedido=" + detalleVentaPedido);
				}
			}
	...
	public void movimientoEntreElmacenes(List<MovimientoEntreAlmacenes> movimientoEntreAlmacenesList, 
			Integer sucursalDestinoId, int tipoAlmacen, Usuario usuarioModifico) {
		...
		try {
			...
			for (MovimientoEntreAlmacenes ma : movimientoEntreAlmacenesList) {
				...
				logger.debug("\t==>>PRODUCTO" + ma.getProducto().getId() + " [" + almacenCentral.getId() + "] -> " + ma.getDestinoSurtir() + 
						"x[" + ma.getProducto() + "] -> [" + almacenDestino.getId() + "]");
				em.flush();
				apDestino = null;
			}
		...
		try {
			...
			if (detalleVentaPedidoCollection != null) {
				for (PedidoVentaDetalle dvp : detalleVentaPedidoCollection) {
					...
					//em.merge(detalleVentaPedido);
					//em.flush();
					logger.debug("\t->crearPedidoCapturado: detalleVentaPedido[" + dvp.getCantidad() + " x " + dvp.getProducto() + "], pedidoVenta=" + 
							pedidoVenta + ": persisted->>detalleVentaPedido=" + detalleVentaPedido);
				}
			}
	...
	public void surtirPedido(PedidoVenta pedidoVenta, Usuario usuarioModifico) throws IllegalStateException {
		...
		try {
			...
			for (PedidoVentaDetalle detalleVentaPedido : detalleVentaPedidoOldCollection) {
				...
				logger.debug("-->> DESCARGA INVENTARIO: almacen:" + almacenProducto.getAlmacen().getId() + ", producto=" + productoASurtir.getCodigoBarras() + ", cantidadPedida=" + 
						detalleVentaPedido.getCantidad() + ", almacenPrincipal = " + almacenProducto.getCantidadActual() + " = " + cantEnAlmacenTotal);
